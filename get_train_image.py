...
def get_5_min_data(ts_code, trade_date, n=4):
    trade_date_list = get_trade_date_list(trade_date, n + 4)
    ...
    five_mininte_data = five_mininte_data.head(int(n * 9))
    five_mininte_data = five_mininte_data.fillna(-1)
...
def get_bak_daily_data(ts_code, trade_date, n=64):
    trade_date_list = get_trade_date_list(trade_date, n + 3)
...
def get_n_fina_indicator_data(ts_code, trade_date, n=16):
    trade_date_list = get_trade_date_list(trade_date, n * 100)
    fina_indicator_data = pro.fina_indicator(ts_code=ts_code, start_date=trade_date_list[-1],
                                             end_date=trade_date_list[0])
...
def get_second_input_image(ts_code, trade_date):
    ...
    ##个股资金流向数据的参数列表
    moneyflow_param_list = ['buy_sm_vol', 'buy_sm_amount', 'sell_sm_vol', 'sell_sm_amount', 'buy_md_vol',
...
def get_valuation_image(ts_code, trade_date):
    fina_indicator_mean_std = pd.read_csv('fina_indicator_mean_std.csv')
    out_image = np.zeros([22, 22, 3], dtype="float32")
    ...
    if fina_indicator_data.shape[0] > 3:
        fina_indicator_data = fina_indicator_data.head(3)
    for i in range(fina_indicator_data.shape[0]):
        for j in range(len(fina_indicator_param_list)):
            param = fina_indicator_param_list[j]
            param_data = (fina_indicator_data[param][i] - fina_indicator_mean_std[param][0])/fina_indicator_mean_std[param][2]
            param_data = param_data*64.0
            m = int(j/11)
            n = j%11
            out_image[2*m+1][2*n+1][i] = param_data
    # print(out_image)
    out_image = cv2.resize(out_image, [128, 128])
    return out_image
...
def rename_valuation_image(folder):
    image_names = tools.get_image_names(folder)
    for i in range(len(image_names)):
        print(float(i) / len(image_names))
        image_name = image_names[i]
        try:
            ...
            # #求取上两个季度的股价收盘价格最大值
            # start_date, _ = tools.get_season_border(trade_date, -2)
            ...
            image_name_new = image_name.replace('.png', '_' + str(class_label) + '.png')
...
if __name__ == "__main__":
    ...
    # train(2000)
    # rename_output_image('valuation')
    # resize_images('valuation3')
    rename_valuation_image('valuation')
    # image_names = tools.get_image_names('valuation2')
    # for i in range(len(image_names)):
    #     print(float(i)/len(image_names))
    #     image_name = image_names[i]
    #     image = cv2.imread(image_name)
    ...
    # image_names = tools.get_image_names('valuation')
    # for i in range(len(image_names)):
    #     print(float(i)/len(image_names))
    #     image_name = image_names[i]
    #     image_split = image_name.split('_')
    #     image_name_new = image_split[0] + '_' + image_split[1] + '_' + image_split[2] + '.png'
    #     os.rename(image_name, image_name_new)
